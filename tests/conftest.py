# This file is for fixtures. Any fixture present in this file will be accessble within the package
# and sub-package, without importing this file

from sqlalchemy.sql.functions import mode
from fastapi.testclient import TestClient
import pytest
from app.main import app
from app.config import settings
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker
from sqlalchemy.ext.declarative import declarative_base
from app.database import get_db, Base
from app.oauth2 import create_access_token
from app import models


SQLALCHEMY_DATABASE_URL = f"postgresql://{settings.database_username}:{settings.database_password}@{settings.database_hostname}:{settings.database_port}/{settings.database_name}_test"
# SQLALCHEMY_DATABASE_URL = "postgresql://user:password@postgresserver/db"

engine = create_engine(
    SQLALCHEMY_DATABASE_URL
)
TestingSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)

#Base.metadata.create_all(bind=engine) # This command will create all the tables in the fastapi_test database


# Dependency
# def override_get_db():
#     db = TestingSessionLocal()
#     try:
#         yield db
#     finally:
#         db.close()



#client = TestClient(app)

@pytest.fixture()
def session():
    # drop all the tables first and then create all the tables for testing using sqlalchemy
    # we can do the same with alembic as well i.e. first downgrade to a specific version and then upgrade to latest version
    Base.metadata.drop_all(bind=engine)
    Base.metadata.create_all(bind=engine)

    db = TestingSessionLocal()
    try:
        yield db
    finally:
        db.close()

@pytest.fixture()
def client(session):
    # run some code before the test execution

    def override_get_db():
        try:
            yield session
        finally:
            session.close()
            
    app.dependency_overrides[get_db] = override_get_db

    yield TestClient(app)
    # yeild is like return to pass a value to calling function. If we use return, the code after this line will not execute.
    # run some code after the test execution, if required


@pytest.fixture
def test_user(client):
    user_data = {"email": "abcdefg@gmail.com", "password": "password123", "phone_number": 99}
    res = client.post("/users/", json = user_data)
    assert res.status_code == 201
    new_user = res.json()
    new_user['password'] = user_data['password']
    return new_user


@pytest.fixture
def token(test_user):
    return create_access_token({"user_id": test_user['id']})

@pytest.fixture
def authorized_client(client, token):
    client.headers = {
        **client.headers,
        "Authorization": f"Bearer {token}"
    }

    return client


@pytest.fixture
def test_posts(test_user, session):
    posts_data = [{
        "title": "first title",
        "content": "first content",
        "owner_id": test_user['id']
    }, {
        "title": "second title",
        "content": "second content",
        "owner_id": test_user['id']        
    }, {
        "title": "third title",
        "content": "third content",
        "owner_id": test_user['id']         
    }]

    def create_post_model(post):
        print("Value of post passed is: ", post)
        return models.Post(**post)

    post_map = map(create_post_model, posts_data)
    # print("Post Map is: ", post_map.value())
    posts = list(post_map)
    print("posts are: ", posts)

    session.add_all(posts)
    
    # session.add_all([models.Post(title="first title", content="first content", owner_id=test_user['id']),
    #                  models.Post(title="second title", content="second content", owner_id=test_user['id']),
    #                  models.Post(title="third title", content="third content", owner_id=test_user['id'])])

    session.commit()

    posts = session.query(models.Post).all()
    print(posts)
    return posts